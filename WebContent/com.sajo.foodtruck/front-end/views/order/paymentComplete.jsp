				...
				<div class="cont_inner">
					<div class="payment_wrap order_view">
						<div class="left">
							...
							<table class="table">
								<tbody>
									<c:forEach var="dto" items="${order}" varStatus="loop">
									<c:if test="${loop.last}">
						...
						<div class="right">
							...
							<ul class="od_menu_list">
										...
										<c:forEach var="dto2" items="${order}" varStatus="loop2">
							...
							<ul class="od_pay_price">
									<c:forEach var="dto3" items="${order}" varStatus="loop3">
										<c:if test="${loop3.last}">
											...
											<li class="last">
												<span>결제 방식</span>
												<strong id="lastStrong">
												<c:choose>
													<c:when test="${dto3.ordertype eq 1}">
													휴대폰 소액결제
													</c:when>
													<c:when test="${dto3.ordertype eq 2}">
													신용카드(온라인결제)
													</c:when>
													<c:when test="${dto3.ordertype eq 3}">
													신용카드(현장결제)
													</c:when>
													<c:when test="${dto3.ordertype eq 4}">
													현금결제(현장결제)
													</c:when>
												</c:choose>
												</strong>
								...
								<div class="m_fixed_btn button_wrap">
									<a href="<c:url value='/Allevent.event'/>" class="button h50 w200 btn_red" id="btn_finishOrder">돌아가기</a>
								</div>
							</div>
						</div>
					</div>
